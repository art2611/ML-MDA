...
# File code imported from https://github.com/bethgelab/imagecorruptions and modifications to handle thermal corruptions
...
def gaussian_noise(x, severity=1, modality="RGB"):
    c = [.08, .12, 0.18, 0.26, 0.38][severity - 1]

    x = np.array(x) / 255.
    channels = 3 if modality == "RGB" else 1

    return np.clip(x + np.random.normal(size=(x.shape[0], x.shape[1], channels), scale=c), 0, 1) * 255


def shot_noise(x, severity=1, modality="RGB"):
...
def impulse_noise(x, severity=1, modality="RGB"):
    ...
    if modality == "IR":
        x = x[:,:,0]
        x = np.clip(sk.util.random_noise(x, mode='s&p', amount=c), 0, 1) * 255
        return np.stack((x,)*3, axis=-1)

    return np.clip(sk.util.random_noise(x, mode='s&p', amount=c), 0, 1) * 255


def speckle_noise(x, severity=1, modality="RGB"):
    c = [.15, .2, 0.35, 0.45, 0.6][severity - 1]

    x = np.array(x) / 255.
    channels = 3 if modality == "RGB" else 1
    return np.clip(x + x * np.random.normal(size=(x.shape[0], x.shape[1], channels), scale=c), 0, 1) * 255


def gaussian_blur(x, severity=1, modality="RGB"):
...
def glass_blur(x, severity=1, modality="RGB"):
...
def defocus_blur(x, severity=1, modality="RGB"):
...
def motion_blur(x, severity=1, modality="RGB"):
...
def zoom_blur(x, severity=1, modality="RGB"):
...
def fog(x, severity=1, modality="RGB"):
...
def frost(x, severity=1, modality="RGB"):
    c = [(1, 0.4),
         (0.8, 0.6),
         (0.7, 0.7),
         (0.65, 0.7),
    ...
    if modality == "IR":
        x = np.clip(rgb2gray(c[0] * np.array(x)) + rgb2gray(c[1] * frost_rescaled), 0, 255)
        return np.stack((x,)*3, axis=-1)
...
def snow(x, severity=1, modality="RGB"):
...
def spatter(x, severity=1, modality="RGB"):
    ...
    if c[5] == 0:
        ...
        if len(x.shape) < 3 or x.shape[2] < 3:
            ...
            x *= (1 - m[..., np.newaxis])
            if modality == "IR":
                x = np.clip(x[:, :, 0] + rgb2gray(color), 0, 1) * 255
                return np.stack((x,) * 3, axis=-1)
            return np.clip(x + color, 0, 1) * 255


def contrast(x, severity=1, modality="RGB"):
...
def brightness(x, severity=1, modality="RGB", decrease = 1):
    c = [.1, .2, .3, .4, .5][severity - 1]
    c = c*decrease # If decrease = -1, will darkens the image.
    x = np.array(x) / 255.
...
def saturate(x, severity=1, modality="RGB"):
...
def jpeg_compression(x, severity=1, modality="RGB"):
...
def pixelate(x, severity=1, modality="RGB"):
...
def elastic_transform(image, severity=1, modality="RGB"):
...
# Added corruption for thermal modality, so that there is the same ratio of each corruption
# ( randomly selected among 20 for RGB and 20 for IR)

def none(x, severity=1, modality="RGB"):
    return x

# Code from https://github.com/MinghuiChen43/CIL-ReID/blob/main/datasets/make_dataloader.py
def rain(image, severity=1, modality="RGB"):
    if severity == 1:
        type = 'drizzle'
    ...
    rain = abm.Compose([
        ...
        RandomBrightness(limit=[bright_value,bright_value], always_apply=True)])
        # abm.augmentations.transforms.RandomBrightnessContrast(brightness_limit=bright_value, contrast_limit=0, always_apply=True)])
