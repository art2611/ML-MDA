...
class TripletSemihardLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # losses(x, y) = max(0, -y * (x1 - x2) + margin)
...
class TripletLoss(nn.Module):
    ...
    For margin = 0. , which implemented as Batch Hard Soft Margin Triplet losses
    Reference:
    ...
    def __init__(self, margin=0.3, mutual_flag=False):
        ...
        if margin == 0.:
            ...
            print('Using soft margin triplet losses')
        else:
    ...
    def forward(self, inputs, targets):
        ...
        # losses = self.ranking_loss(dist_an, dist_ap, y)
        if self.margin == 0.:
