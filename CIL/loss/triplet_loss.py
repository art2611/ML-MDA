...
class TripletLoss(object):
    """
    Triplet losses using HARDER example mining,
    modified based on original triplet losses using hard example mining
    """
...
def softmax_weights(dist, mask):
    ...
    diff = dist - max_v
    Z = torch.sum(torch.exp(diff) * mask, dim=1,
                  keepdim=True) + 1e-6  # avoid division by zero
